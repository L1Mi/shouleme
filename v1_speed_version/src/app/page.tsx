...
import { Camera, Circle, Calendar, BarChart3, User } from 'lucide-react'
...
export default function Home() {
  ...
  const goalBgColor = { 'lose_fat': 'bg-emerald-500', 'muscle': 'bg-blue-500', 'maintain': 'bg-gray-400' }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      {/* 顶部 - 柔和渐变 */}
      <div className="bg-gradient-to-b from-emerald-400 to-emerald-100 text-gray-800 px-4 pt-12 pb-8">
        <div className="flex justify-between items-center mb-4">
          <div></div>
          <h1 className="text-xl font-bold text-gray-800">瘦了吗</h1>
          <Link href="/profile" className="p-2 bg-white/50 rounded-full">
        ...
        <div className="flex justify-center mb-4">
          <span className={`inline-block px-4 py-1.5 rounded-full text-sm font-medium text-white ${goalBgColor[user.goal]}`}>
            {goalText[user.goal]}
          </span>
        </div>
        ...
        <div className="flex justify-center mb-4">
          <div className="relative w-44 h-44">
            <svg className="w-full h-full transform -rotate-90">
              <circle cx="88" cy="88" r={circleRadius} stroke="rgba(0,0,0,0.1)" strokeWidth="10" fill="none" />
              <circle
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <div className="text-4xl font-bold text-gray-800">{caloriesConsumed}</div>
              <div className="text-gray-500 text-xs">/ {caloriesData.target} 千卡</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-center gap-12">
          <div className="text-center">
            <div className="text-2xl font-bold text-gray-800">{caloriesRemaining > 0 ? caloriesRemaining : 0}</div>
            <div className="text-gray-500 text-xs">剩余</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-gray-800">{user.weight}</div>
            <div className="text-gray-500 text-xs">kg</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-gray-800">{caloriesData.target}</div>
      ...
      <div className="px-4 -mt-6">
        <button className="w-full bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white py-4 rounded-2xl shadow-lg flex items-center justify-center gap-3 transition-all active:scale-95">
            ...
            <div key={record.id} className="bg-white rounded-2xl p-4 shadow-sm">
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  <div className="font-bold text-emerald-600 text-lg">{record.calories}</div>
      ...
      {/* 底部导航 */}
      <div 
        className={`fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-6 py-3 flex justify-around items-center transition-transform duration-300 shadow-lg ${
