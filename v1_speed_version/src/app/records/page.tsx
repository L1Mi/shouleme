  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
          ...
          <div className="bg-white/20 rounded-full p-1 flex">
            <button
              ...
              className={`px-6 py-2 rounded-full text-sm font-medium transition-all ${
                view === 'week' ? 'bg-white text-emerald-600' : 'text-white/80'
              }`}
            >
              周
            </button>
            <button
              ...
              className={`px-6 py-2 rounded-full text-sm font-medium transition-all ${
                view === 'month' ? 'bg-white text-emerald-600' : 'text-white/80'
              }`}
            >
              月
            </button>
      ...
      <div className="p-4 -mt-4">
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white rounded-2xl p-4 shadow-sm text-center">
            <div className="text-2xl font-bold text-emerald-600">{Math.round(avgCalories)}</div>
            <div className="text-xs text-gray-500">平均千卡</div>
          </div>
          <div className="bg-white rounded-2xl p-4 shadow-sm text-center">
            <div className={`text-2xl font-bold ${totalWeightLoss > 0 ? 'text-emerald-600' : 'text-red-500'}`}>
            ...
            {data.map((d, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center">
                  <div 
                    className={`w-full rounded-t-lg ${isOver ? 'bg-red-400' : 'bg-emerald-500'}`}
                    style={{ height: `${height}%` }}
                  ></div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-6 py-3 flex justify-around items-center">
        ...
        <Link href="/records" className="flex flex-col items-center text-emerald-600">
